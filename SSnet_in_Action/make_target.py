...
'''
    Paper Sec 2.1
    A protein can be represented by the alpha carbons (CA atoms) 
    of the backbone as it defines a unique and recognizable structure, 
    especially for protein categorization.
    The secondary structure information can be retrieved by
    a smooth curve generated by a cubic spline fit of the CA atoms.
    
    the arc length s,
    scalar curvature κ （a function of s）
    scalar torsion τ which define the 3D curve r(s)
    
'''

# alpha carbons (CA atoms) 
...
def get_init_kt(x,y,z):
   ...
   curvatures = [] # scalar curvature κ （a function of arc length s）
   ...
   for i in range(len(x_3rd)):
       # the scalar torsion calculation [scalar torsion τ which define the 3D curve r(s)]
...
def get_kt(i, d): #todo: 1. read the pdb file
   # i: the path of a pdb file for a protein, input file
